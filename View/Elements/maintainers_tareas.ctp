...
<div class="informer informer-info"><?= $contTareas = (!empty($tareasNotificacion)) ? count($tareasNotificacion) : 0 ; ?></div>
<div class="panel panel-primary animated zoomIn xn-drop-left xn-panel-dragging">
    <div class="panel-heading">
        <h3 class="panel-title"><span class="fa fa-tasks"></span> Tareas en progreso</h3>
        <div class="pull-right">
            <span class="label label-info"><?=$contTareas = (!empty($tareasNotificacion)) ? count(Hash::extract($tareasNotificacion, '{n}.Tarea.en_progreso') ) . ' tarea en progreso' : '0 tareas en progreso' ; ?></span>
        </div>
    </div>
    ...
    <? foreach ($tareasNotificacion as $index => $tarea) : ?>
        ...
        <? endif; ?>
        	<?=$this->Html->link(
                ...
                array('controller' => 'tareas', 'action' => 'work', $tarea['Tarea']['id']), array('escape' => false, 'class' => 'list-group-item')); ?>
    <? endforeach; ?>
    </div>
